...
class GeoCtub extends THREE.Geometry {

    constructor({ dz, rmin, rmax, phi1, phi2, lx, ly, lz, hx, hy, hz, twist, numSegs }) {
        super();
        this.parameters = { dz, rmin, rmax, phi1, phi2, lx, ly, lz, hx, hy, hz, twist, numSegs };
        this.type = 'GeoCtub';
        this.fromBufferGeometry( new GeoCtubBuffer({ dz, rmin, rmax, phi1, phi2, lx, ly, lz, hx, hy, hz, twist, numSegs }) );
    }

}

class GeoCtubBuffer extends THREE.BufferGeometry {

    constructor({ dz = 50, rmin = 20, rmax = 40, phi1 = 0, phi2 = 270, lx, ly, lz, hx, hy, hz, twist = false, numSegs = 12 }) {
        super();
		if ( lx === undefined && ly === undefined && lz === undefined ) {
			let theta = 160.0 * Math.PI / 180.0, phi = 30.0 * Math.PI / 180.0;
			lx = Math.sin( theta ) * Math.cos( phi );
			ly = Math.sin( theta ) * Math.sin( phi );
			lz = Math.cos( theta );
		}
		if ( hx === undefined && hy === undefined && hz === undefined ) {
			let theta = 20.0 * Math.PI / 180.0, phi = 60.0 * Math.PI / 180.0;
			hx = Math.sin( theta ) * Math.cos( phi );
			hy = Math.sin( theta ) * Math.sin( phi );
			hz = Math.cos( theta );
		}
        this.parameters = { dz, rmin, rmax, phi1, phi2, lx, ly, lz, hx, hy, hz, numSegs };
        this.type = 'GeoCtubBuffer';
		...
		for ( i = 0; i < vertices.length; i += 3 ) {
			if ( vertices[ i + 2 ] < 0 ) {
				vertices[ i + 2 ] = -dz - ( vertices[ i ] * lx + vertices[ i + 1 ] * ly ) / lz;
			} else {
				vertices[ i + 2 ] = dz - ( vertices[ i ] * hx + vertices[ i + 1 ] * hy ) / hz;
			}
		}
